...
global lamda nu1 nu2 mu1 mu2 pp method IndexTest;
    ...
    YY_1BMC=[W.*matrix; W.*matrix;  W.*matrix];%[W.*matrix; sqrt(nu1)*Z';  sqrt(nu2)*Y];
     ...
     if strcmp(method,'NNM')
         switch (IndexTest)
             ...
             case 2
                [X] = FNNM( YY,AA,size(matrix) ,0,lamda);
             case 3
                [X]=WWFNNM( YY,AA,size(matrix) ,0,lamda);
             case 4
                [X]=WWNNM_ALM( YY,AA, 0,size(matrix));                
             case 5
                [X] = WLRAppro( YY, AA, 0, lamda ,size(matrix));
             case 6
                 [X]=WWNNM( YY,AA, 0,size(matrix));
         end
